...
bool InputGeom::loadMesh(rcContext* ctx, const char* filepath)
{
	if (m_mesh)
	{
		rcFreeChunkyTriMesh(m_chunkyMesh);	// Modified by RangerUFO
		m_chunkyMesh = 0;
		rcFreeMeshLoaderObj(m_mesh);	// Modified by RangerUFO
		m_mesh = 0;
	}
	m_offMeshConCount = 0;
	m_volumeCount = 0;
	
	m_mesh = rcAllocMeshLoaderObj();	// Modified by RangerUFO
	if (!m_mesh)
	{
	...
	m_chunkyMesh = rcAllocChunkyTriMesh();	// Modified by RangerUFO
	if (!m_chunkyMesh)
	{
...
// Added by RangerUFO
// @{
InputGeom* rcAllocInputGeom()
{
	void* p = rcAlloc(sizeof(InputGeom), RC_ALLOC_PERM);
	if (!p) return 0;
	return ::new(p) InputGeom;
}

void rcFreeInputGeom(InputGeom* p)
{
	if (!p) return;
	p->~InputGeom();
	rcFree(p);
